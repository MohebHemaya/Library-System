...
const TransactionList = () => {
  ...
  const fetchData = () => {
    ...
    Promise.all([
      ...
      .then(([booksRes, membersRes, transactionsRes]) => {
        ...
        const uniqueCategories = [...new Set(bookData.filter(book => book.category).map(book => book.category))];
  ...
  useEffect(() => {
    if (selectedMember && memberMap[selectedMember]) {
      ...
      const avgBookCost = books.length > 0 
        ? books.reduce((sum, book) => sum + (book.debtCost || 50), 0) / books.length 
        : 50;
  ...
  }, [selectedMember, selectedBooks, memberMap, bookMap, books, MAX_DEBT_LIMIT]);
  ...
  const handleLendBooks = () => {
    ...
    if (member.debt + totalCost > MAX_DEBT_LIMIT) {
      alert(`Member debt cannot exceed EGP ${MAX_DEBT_LIMIT}. Current debt: EGP ${member.debt}, New transaction cost: EGP ${totalCost}`);
      return;
    }
    ...
    axios.patch(`http://localhost:8000/members/${member.id}`, {
      ...
      .then(() => {
        ...
        Promise.all(
          selectedBooks.map((bookId) => {
            ...
            return axios.patch(`http://localhost:8000/books/${bookId}`, { 
            ...
            Promise.all(
              ...
              .then((responses) => {
                ...
                if (filterStatus === 'all' || filterStatus === 'active') {
                  const updatedFiltered = [...filteredTransactions, ...newTransactionData];
                  ...
                  setCurrentPage(Math.ceil(updatedFiltered.length / itemsPerPage));
                }
  ...
  const handleReturnBook = (transactionId) => {
    ...
    axios
      .patch(`http://localhost:8000/members/${member.id}`, {
      ...
      .then(() => {
        ...
        axios
          .patch(`http://localhost:8000/books/${transaction.bookID}`, { 
            availableCopies: newAvailableCopies,
          ...
          .then(() => {
            ...
            axios
              .patch(`http://localhost:8000/transactions/${transactionId}`, {
                returned: true
              })
              .then((response) => {
                ...
                if (filterStatus === 'active') {
                  const updatedFiltered = filteredTransactions.filter(t => t.id !== transactionId);
                  setFilteredTransactions(updatedFiltered);
        ...
        <div className="card mb-4 animate-slideIn">
          ...
          <div className="card-body">
                    ...
                    {filteredMembersForLending.length === 0 ? (
                      ...
                      <div className="list-group list-group-flush">
                        {filteredMembersForLending.map((member) => (
                          ...
                          >
                            <div>
                              <div className="fw-bold"><i className="fas fa-user me-2"></i> {member.name}</div>
                              {member.email && (
                                <div className="small text-muted">
                                  <i className="fas fa-envelope me-1"></i> {member.email}
                                </div>
                              )}
                              {member.phone && (
                                <div className="small text-muted">
                              ...
                              <div>
                                <span className="badge bg-secondary me-2">
                                  <i className="fas fa-pound-sign me-1"></i> {member.debt}
                                </span>
                                {member.debt === 0 ? (
                                  <span className="badge bg-success">No Debt</span>
                                ) : member.debt >= (member.maxDebt || MAX_DEBT_LIMIT) * 0.8 ? (
                                  <span className="badge bg-danger">High Debt</span>
                                ) : member.debt >= (member.maxDebt || MAX_DEBT_LIMIT) * 0.5 ? (
                                  <span className="badge bg-warning text-dark">Medium Debt</span>
                                ) : (
                                  <span className="badge bg-info">Low Debt</span>
                                )}
                              </div>
                            </div>
                            <div>
                      ...
                      <div className="mt-2">
                        <div className="progress" style={{ height: "8px" }}>
                          <div 
                            ...
                            role="progressbar" 
                            style={{ width: `${getDebtPercentage(memberMap[selectedMember].debt)}%` }}
                        ...
                        <div className="d-flex justify-content-between">
                          <small>Debt: EGP {memberMap[selectedMember].debt}</small>
                        ...
                        {filteredBooksForLending.map((book) => (
                          ...
                          >
                            <div>
                              ...
                              <div>
                                <span className="badge bg-secondary me-2">
                                  <i className="fas fa-pound-sign me-1"></i> {book.debtCost || 50}
                                </span>
                  ...
                  <div className="mt-2">
                    ...
                    <ul className="list-group">
                      {selectedBooks.map(bookId => {
                        const book = bookMap[bookId];
                        return book ? (
                          <li key={bookId} className="list-group-item d-flex justify-content-between align-items-center py-2">
                            <div>
            ...
            <div className="d-flex justify-content-end mt-3">
              <button className="btn btn-secondary me-2" onClick={() => setShowLendForm(false)}>
                Cancel
              </button>
              <button 
            ...
            {selectedMember && selectedBooks.length > 0 && memberMap[selectedMember] && (
              <div className="alert alert-info mt-3">
                <p className="mb-0"><strong>Transaction Summary:</strong></p>
                <p className="mb-1">Member: {memberMap[selectedMember].name}</p>
                <p className="mb-1">Books: {selectedBooks.length}</p>
                <p className="mb-1">Total Cost: EGP {calculatedDebt.totalCost}</p>
                <p className="mb-1">Current Debt: EGP {memberMap[selectedMember].debt}</p>
                <p className="mb-0">New Total Debt: EGP {calculatedDebt.newTotalDebt}</p>
                {calculatedDebt.newTotalDebt > MAX_DEBT_LIMIT && (
                  <div className="alert alert-danger mt-2 mb-0">
                    <i className="fas fa-exclamation-triangle me-2"></i>
                    <strong>Warning:</strong> This transaction will exceed the maximum debt limit of EGP {MAX_DEBT_LIMIT}.
                  </div>
                )}
              </div>
            )}
        ...
        <tbody>
              ...
              <td>{transaction.id}</td>
                      ...
                      <td>
                        ...
                        {memberMap[transaction.memberID]?.name || `Member ID: ${transaction.memberID}`}
                      </td>
                      <td>
              ...
              return (
                ...
                >
                  <div className="card-header d-flex justify-content-between align-items-center">
                    <h5 className="mb-0">
                      <i className="fas fa-exchange-alt me-2"></i> Transaction #{transaction.id}
                    </h5>
                    {transaction.returned ? (
                      <span className="badge bg-success">
                        <i className="fas fa-check-circle me-1"></i> Returned
                  ...
                  <div className="card-body">
                    <div className="mb-3">
                      ...
                      <div className="d-flex align-items-center">
                        ...
                        <strong>{book?.title || `Book ID: ${transaction.bookID}`}</strong>
                      </div>
                    </div>
                    <div className="mb-3">
                      ...
                      <div className="d-flex align-items-center">
                        ...
                        <strong>{memberMap[transaction.memberID]?.name || `Member ID: ${transaction.memberID}`}</strong>
                      </div>
                    </div>
