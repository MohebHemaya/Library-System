// Initial database data
const initialData = {
  books: [
    {
      "id": "8197",
      "title": "Writings to Young Women from Laura Ingalls Wilder: On Wisdom and Virtues (Writings to Young Women on Laura Ingalls Wilder #1)",
      "authors": "Laura Ingalls Wilder/Stephen W. Hines",
      "average_rating": "3.99",
      "isbn": "1400307848",
      "isbn13": "9781400307845",
      "language_code": "eng",
      "num_pages": "225",
      "ratings_count": "108",
      "text_reviews_count": "11",
      "publication_date": "5/10/2006",
      "publisher": "Tommy Nelson",
      "lent": false,
      "totalCopies": 5,
      "availableCopies": 5,
      "debtCost": 50,
      "category": "Self-Help"
    },
    {
      "id": "44300",
      "title": "Smart Discipline: Fast  Lasting Solutions for Your Child's Self-Esteem and Your Peace of Mind",
      "authors": "Larry J. Koenig",
      "average_rating": "3.99",
      "isbn": "0060936665",
      "isbn13": "9780060936662",
      "language_code": "en-US",
      "num_pages": "208",
      "ratings_count": "13",
      "text_reviews_count": "2",
      "publication_date": "3/2/2004",
      "publisher": "William Morrow Paperbacks",
      "lent": false,
      "totalCopies": 3,
      "availableCopies": 3,
      "debtCost": 45,
      "category": "Psychology"
    },
    {
      "id": "43877",
      "title": "The Monk Who Sold His Ferrari: A Fable About Fulfilling Your Dreams and Reaching Your Destiny",
      "authors": "Robin S. Sharma",
      "average_rating": "3.83",
      "isbn": "0062515675",
      "isbn13": "9780062515674",
      "language_code": "eng",
      "num_pages": "208",
      "ratings_count": "71133",
      "text_reviews_count": "3741",
      "publication_date": "4/21/1999",
      "publisher": "HarperOne",
      "lent": true,
      "totalCopies": 10,
      "availableCopies": 1,
      "debtCost": 60,
      "category": "Self-Help"
    },
    {
      "id": "18745",
      "title": "Female Chauvinist Pigs: Women and the Rise of Raunch Culture",
      "authors": "Ariel Levy",
      "average_rating": "3.68",
      "isbn": "0743284283",
      "isbn13": "9780743284288",
      "language_code": "en-US",
      "num_pages": "236",
      "ratings_count": "8511",
      "text_reviews_count": "765",
      "publication_date": "10/3/2006",
      "publisher": "Free Press",
      "lent": false,
      "totalCopies": 4,
      "availableCopies": 4,
      "debtCost": 55,
      "category": "Sociology"
    },
    {
      "id": "5552",
      "title": "QED: The Strange Theory of Light and Matter",
      "authors": "Richard P. Feynman",
      "average_rating": "4.24",
      "isbn": "0691024170",
      "isbn13": "9780691024172",
      "language_code": "eng",
      "num_pages": "158",
      "ratings_count": "13463",
      "text_reviews_count": "320",
      "publication_date": "10/21/1988",
      "publisher": "Princeton University Press",
      "lent": true,
      "totalCopies": 7,
      "availableCopies": 3,
      "debtCost": 65,
      "category": "Science"
    },
    {
      "id": "676",
      "title": "Sailing Alone Around the Room: New and Selected Poems",
      "authors": "Billy Collins",
      "average_rating": "4.23",
      "isbn": "0375755195",
      "isbn13": "9780375755194",
      "language_code": "eng",
      "num_pages": "192",
      "ratings_count": "12180",
      "text_reviews_count": "630",
      "publication_date": "9/17/2002",
      "publisher": "Random House Trade Paperbacks",
      "lent": true,
      "totalCopies": 2,
      "availableCopies": 1,
      "debtCost": 40,
      "category": "Poetry"
    },
    {
      "id": "39749",
      "title": "Memoranda",
      "authors": "Jeffrey Ford/Jacques Guiod",
      "average_rating": "3.90",
      "isbn": "2290319708",
      "isbn13": "9782290319703",
      "language_code": "fre",
      "num_pages": "253",
      "ratings_count": "206",
      "text_reviews_count": "13",
      "publication_date": "12/10/2002",
      "publisher": "J'ai Lu",
      "lent": false,
      "totalCopies": 6,
      "availableCopies": 6,
      "debtCost": 50,
      "category": "Fiction"
    },
    {
      "id": "3690",
      "title": "The Power and the Glory",
      "authors": "Graham Greene/John Updike",
      "average_rating": "4.00",
      "isbn": "0142437301",
      "isbn13": "9780142437308",
      "language_code": "eng",
      "num_pages": "222",
      "ratings_count": "25490",
      "text_reviews_count": "1585",
      "publication_date": "2/25/2003",
      "publisher": "Penguin Books",
      "lent": false,
      "totalCopies": 8,
      "availableCopies": 8,
      "debtCost": 55,
      "category": "Fiction"
    },
    {
      "id": "1725",
      "title": "The Art of Love and Other Poems",
      "authors": "Ovid/J.H. Mozley/G.P. Goold",
      "average_rating": "4.02",
      "isbn": "0674992555",
      "isbn13": "9780674992559",
      "language_code": "eng",
      "num_pages": "400",
      "ratings_count": "98",
      "text_reviews_count": "5",
      "publication_date": "1/1/1929",
      "publisher": "Harvard University Press",
      "lent": false,
      "totalCopies": 3,
      "availableCopies": 3,
      "debtCost": 70,
      "category": "Poetry"
    },
    {
      "id": "17828",
      "title": "The Master and Margarita",
      "authors": "Mikhail Bulgakov/Michael Karpelson",
      "average_rating": "4.30",
      "isbn": "1411683056",
      "isbn13": "9781411683051",
      "language_code": "eng",
      "num_pages": "332",
      "ratings_count": "493",
      "text_reviews_count": "47",
      "publication_date": "4/1/2006",
      "publisher": "Lulu Press",
      "lent": false,
      "totalCopies": 5,
      "availableCopies": 5,
      "debtCost": 60,
      "category": "Fiction"
    },
    {
      "id": "8205",
      "title": "Writings to Young Women on Laura Ingalls Wilder: As Told By Her Family  Friends  and Neighbors (Writings to Young Women on Laura Ingalls Wilder #3)",
      "authors": "Laura Ingalls Wilder/Stephen W. Hines",
      "average_rating": "3.98",
      "isbn": "1400307864",
      "isbn13": "9781400307869",
      "language_code": "eng",
      "num_pages": "128",
      "ratings_count": "41",
      "text_reviews_count": "2",
      "publication_date": "5/8/2006",
      "publisher": "Thomas Nelson",
      "lent": false,
      "totalCopies": 4,
      "availableCopies": 4,
      "debtCost": 45,
      "category": "Self-Help"
    },
    {
      "id": "33513",
      "title": "The White Man's Burden: Why the West's Efforts to Aid the Rest Have Done So Much Ill and So Little Good",
      "authors": "William Easterly",
      "average_rating": "3.83",
      "isbn": "0143038826",
      "isbn13": "9780143038825",
      "language_code": "eng",
      "num_pages": "436",
      "ratings_count": "4473",
      "text_reviews_count": "232",
      "publication_date": "3/1/2007",
      "publisher": "Penguin Books",
      "lent": false,
      "totalCopies": 3,
      "availableCopies": 3,
      "debtCost": 50,
      "category": "Politics"
    },
    {
      "id": "890",
      "title": "Of Mice and Men",
      "authors": "John Steinbeck",
      "average_rating": "3.87",
      "isbn": "0142000671",
      "isbn13": "9780142000670",
      "language_code": "eng",
      "num_pages": "103",
      "ratings_count": "1755253",
      "text_reviews_count": "25554",
      "publication_date": "1/8/2002",
      "publisher": "Penguin Books",
      "lent": true,
      "totalCopies": 12,
      "availableCopies": 8,
      "debtCost": 40,
      "category": "Fiction"
    },
    {
      "id": "15930",
      "title": "Wokini: A Lakota Journey to Happiness and Self-Understanding",
      "authors": "Billy Mills/Nicholas Sparks",
      "average_rating": "3.57",
      "isbn": "1561706604",
      "isbn13": "9781561706600",
      "language_code": "eng",
      "num_pages": "175",
      "ratings_count": "253",
      "text_reviews_count": "21",
      "publication_date": "6/1/2003",
      "publisher": "Hay House",
      "lent": false,
      "totalCopies": 2,
      "availableCopies": 2,
      "debtCost": 40,
      "category": "Self-Help"
    },
    {
      "id": "31389",
      "title": "Life Management for Busy Woman: Growth and Study Guide",
      "authors": "Elizabeth George",
      "average_rating": "4.53",
      "isbn": "0736910190",
      "isbn13": "9780736910194",
      "language_code": "en-GB",
      "num_pages": "166",
      "ratings_count": "14",
      "text_reviews_count": "1",
      "publication_date": "7/1/2002",
      "publisher": "Harvest House Publishers",
      "lent": false,
      "totalCopies": 4,
      "availableCopies": 4,
      "debtCost": 50,
      "category": "Self-Help"
    },
    {
      "id": "26827",
      "title": "Park Profiles: Grand Canyon Country (Park Profiles)",
      "authors": "National Geographic Society",
      "average_rating": "4.10",
      "isbn": "0792270320",
      "isbn13": "9780792270324",
      "language_code": "eng",
      "num_pages": "200",
      "ratings_count": "10",
      "text_reviews_count": "1",
      "publication_date": "1/19/2010",
      "publisher": "National Geographic",
      "lent": false,
      "totalCopies": 6,
      "availableCopies": 6,
      "debtCost": 65,
      "category": "Travel"
    },
    {
      "id": "29879",
      "title": "They Dare to Speak Out: People and Institutions Confront Israel's Lobby",
      "authors": "Paul Findley",
      "average_rating": "4.07",
      "isbn": "155652482X",
      "isbn13": "9781556524820",
      "language_code": "eng",
      "totalCopies": 3,
      "availableCopies": 3,
      "debtCost": 55,
      "category": "Politics"
    },
    {
      "id": "2336",
      "title": "Tandia",
      "authors": "Bryce Courtenay",
      "average_rating": "4.05",
      "isbn": "0140272925",
      "isbn13": "9780140272925",
      "language_code": "eng",
      "num_pages": "905",
      "ratings_count": "8461",
      "text_reviews_count": "369",
      "publication_date": "8/31/1998",
      "publisher": "Penguin Books Australia Ltd.",
      "lent": false,
      "category": "Fiction"
    },
    {
      "id": "44183",
      "title": "Chemistry and Other Stories",
      "authors": "Ron Rash",
      "average_rating": "4.19",
      "isbn": "0312425082",
      "isbn13": "9780312425081",
      "language_code": "eng",
      "num_pages": "230",
      "ratings_count": "601",
      "text_reviews_count": "79",
      "publication_date": "9/5/2000",
      "publisher": "St. Martins Press-3PL",
      "lent": false,
      "category": "Fiction"
    },
    {
      "id": "2166",
      "title": "The Old Man and the Sea",
      "authors": "Ernest Hemingway/Donald Sutherland",
      "average_rating": "3.77",
      "isbn": "0743564367",
      "isbn13": "9780743564366",
      "language_code": "eng",
      "num_pages": "3",
      "ratings_count": "393",
      "text_reviews_count": "77",
      "publication_date": "5/1/2006",
      "publisher": "Simon  Schuster Audio",
      "lent": false,
      "category": "Fiction"
    },
    {
      "id": "c77b",
      "title": "moheb",
      "authors": "moheb",
      "isbn": "",
      "publisher": "moheb",
      "num_pages": "21",
      "category": "Other"
    }
  ],
  members: [
    {
      "id": "454d",
      "name": "Moheb",
      "debt": 0
    },
    {
      "id": "ba9a",
      "name": "abdo",
      "debt": 0
    },
    {
      "id": "7f5f",
      "name": "seif",
      "debt": 0
    }
  ],
  transactions: [
    {
      "id": "67b0",
      "bookID": "8205",
      "memberID": "454d",
      "date": "3/5/2025",
      "returned": true
    },
    {
      "id": "e776",
      "bookID": "44183",
      "memberID": "454d",
      "date": "3/5/2025",
      "returned": true
    },
    {
      "id": "1a25",
      "bookID": "31389",
      "memberID": "ba9a",
      "date": "4/17/2025",
      "returned": true
    },
    {
      "id": "7dc9",
      "bookID": "29879",
      "memberID": "ba9a",
      "date": "4/17/2025",
      "returned": true
    },
    {
      "id": "bfd3",
      "bookID": "8197",
      "memberID": "ba9a",
      "date": "4/17/2025",
      "returned": true
    },
    {
      "id": "e45c",
      "bookID": "44300",
      "memberID": "ba9a",
      "date": "4/17/2025",
      "returned": true
    },
    {
      "id": "4426",
      "bookID": "43877",
      "memberID": "ba9a",
      "date": "4/17/2025",
      "returned": true
    },
    {
      "id": "aa13",
      "bookID": "18745",
      "memberID": "ba9a",
      "date": "4/17/2025",
      "returned": true
    },
    {
      "id": "6d29",
      "bookID": "5552",
      "memberID": "ba9a",
      "date": "4/17/2025",
      "returned": true
    },
    {
      "id": "c691",
      "bookID": "8197",
      "memberID": "454d",
      "date": "4/17/2025",
      "returned": true
    },
    {
      "id": "d02f",
      "bookID": "43877",
      "memberID": "454d",
      "date": "4/17/2025",
      "returned": true,
      "debtCost": 50
    },
    {
      "id": 20,
      "bookID": "890",
      "memberID": "10",
      "date": "2023-09-20",
      "returned": true,
      "debtCost": 40
    },
    {
      "id": 21,
      "bookID": "676",
      "memberID": "1",
      "date": "2023-09-25",
      "returned": true,
      "debtCost": 40
    },
    {
      "id": 22,
      "bookID": "5552",
      "memberID": "2",
      "date": "2023-10-01",
      "returned": true,
      "debtCost": 65
    },
    {
      "id": "b212",
      "bookID": "31389",
      "memberID": "ba9a",
      "date": "4/17/2025",
      "returned": true,
      "debtCost": 50
    },
    {
      "id": "125e",
      "bookID": "39749",
      "memberID": "ba9a",
      "date": "4/17/2025",
      "returned": true,
      "debtCost": 50
    },
    {
      "id": "c942",
      "bookID": "18745",
      "memberID": "ba9a",
      "date": "4/17/2025",
      "returned": true,
      "debtCost": 55
    },
    {
      "id": "15a8",
      "bookID": "44300",
      "memberID": "ba9a",
      "date": "4/17/2025",
      "returned": true,
      "debtCost": 45
    },
    {
      "id": "0271",
      "bookID": "26827",
      "memberID": "454d",
      "date": "4/17/2025",
      "returned": true,
      "debtCost": 65
    },
    {
      "id": "8e23",
      "bookID": "18745",
      "memberID": "454d",
      "date": "4/17/2025",
      "returned": true,
      "debtCost": 55
    },
    {
      "id": "2db9",
      "bookID": "676",
      "memberID": "454d",
      "date": "4/17/2025",
      "returned": true,
      "debtCost": 40
    },
    {
      "id": "a621",
      "bookID": "1725",
      "memberID": "454d",
      "date": "4/17/2025",
      "returned": true,
      "debtCost": 70
    }
  ]
};

// Initialize or get data from localStorage
const initializeDatabase = () => {
  if (typeof window !== 'undefined') {
    const stored = localStorage.getItem('libraryDB');
    if (!stored) {
      localStorage.setItem('libraryDB', JSON.stringify(initialData));
      return initialData;
    }
    return JSON.parse(stored);
  }
  return initialData;
};

// Save data to localStorage
const saveDatabase = (data) => {
  if (typeof window !== 'undefined') {
    localStorage.setItem('libraryDB', JSON.stringify(data));
  }
};

// Get all data
export const getDatabase = () => {
  return initializeDatabase();
};

// Books operations
export const getBooks = () => {
  const db = getDatabase();
  return db.books;
};

export const getBook = (id) => {
  const db = getDatabase();
  return db.books.find(book => book.id === id);
};

export const addBook = (book) => {
  const db = getDatabase();
  const newBook = { ...book, id: Date.now().toString() };
  db.books.push(newBook);
  saveDatabase(db);
  return newBook;
};

export const updateBook = (id, updatedBook) => {
  const db = getDatabase();
  const index = db.books.findIndex(book => book.id === id);
  if (index !== -1) {
    db.books[index] = { ...db.books[index], ...updatedBook };
    saveDatabase(db);
    return db.books[index];
  }
  return null;
};

export const deleteBook = (id) => {
  const db = getDatabase();
  db.books = db.books.filter(book => book.id !== id);
  saveDatabase(db);
};

// Members operations
export const getMembers = () => {
  const db = getDatabase();
  return db.members;
};

export const getMember = (id) => {
  const db = getDatabase();
  return db.members.find(member => member.id === id);
};

export const addMember = (member) => {
  const db = getDatabase();
  const newMember = { ...member, id: Date.now().toString(), debt: 0 };
  db.members.push(newMember);
  saveDatabase(db);
  return newMember;
};

export const updateMember = (id, updatedMember) => {
  const db = getDatabase();
  const index = db.members.findIndex(member => member.id === id);
  if (index !== -1) {
    db.members[index] = { ...db.members[index], ...updatedMember };
    saveDatabase(db);
    return db.members[index];
  }
  return null;
};

export const deleteMember = (id) => {
  const db = getDatabase();
  db.members = db.members.filter(member => member.id !== id);
  saveDatabase(db);
};

// Transactions operations
export const getTransactions = () => {
  const db = getDatabase();
  return db.transactions;
};

export const getTransaction = (id) => {
  const db = getDatabase();
  return db.transactions.find(trans => trans.id === id);
};

export const addTransaction = (transaction) => {
  const db = getDatabase();
  const newTransaction = { ...transaction, id: Date.now().toString() };
  db.transactions.push(newTransaction);
  saveDatabase(db);
  return newTransaction;
};

export const updateTransaction = (id, updatedTransaction) => {
  const db = getDatabase();
  const index = db.transactions.findIndex(trans => trans.id === id);
  if (index !== -1) {
    db.transactions[index] = { ...db.transactions[index], ...updatedTransaction };
    saveDatabase(db);
    return db.transactions[index];
  }
  return null;
};

export const deleteTransaction = (id) => {
  const db = getDatabase();
  db.transactions = db.transactions.filter(trans => trans.id !== id);
  saveDatabase(db);
};
